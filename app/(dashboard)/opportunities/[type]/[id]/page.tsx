...
export default function OpportunityDetailPage() {
  ...
  // App Store: mock skills for match score + store fallback for projects
  ...
  // Role checks
  const isStaff = ["tnp", "recruiter", "teacher"].includes(
    user?.username?.toLowerCase() || ""
  )
  // File 1 only checked "student"; File 2 also included "anshi_student" — keeping both
  const isStudent =
    user?.username === "student" || user?.username === "anshi_student"

  // ─── Effect 1: Fetch the opportunity ────────────────────────────────────────
  useEffect(() => {
    const fetchOpportunity = async () => {
      setLoading(true)
      setError(null)
      ...
      try {
        ...
        if (res.ok) {
          const resData = await res.json()
          // Some endpoints wrap in `.data`, others don't
          ...
          if (type === "internship") {
            setOpportunity({
              ...
              postedBy: data.postedBy
                ? `${data.postedBy.first_name} ${data.postedBy.last_name}`
                : "Unknown",
        ...
        // ── Fallback: check local store (covers projects posted locally) ──────
        const storeOpp = opportunities.find((o) => o.id === id)
        if (storeOpp) {
        ...
        // Network error — still try store fallback
        const storeOpp = opportunities.find((o) => o.id === id)
        if (storeOpp) {
          setOpportunity(storeOpp)
        } else {
    ...
    if (type && id && !userLoading) fetchOpportunity()
  }, [type, id, userLoading, opportunities])

  // ─── Effect 2: Fetch applied groups (staff + project only) ──────────────────
  useEffect(() => {
    const fetchGroups = async () => {
      if (!opportunity || !isStaff || opportunity.type !== "project") return
      try {
        const groupsRes = await fetch(
          `${backendBase}/api/academic-project/${opportunity.id}/groups`
        )
        const groupsData = await groupsRes.json()
        if (groupsRes.ok) setAppliedGroups(groupsData.data || [])
      } catch (err) {
  ...
  const isPlaced =
    opportunity?.applications?.some(
      (app: any) =>
        app?.student?.user_id === user?.id && app?.status === "SELECTED"
    ) || false

  const matchScore =
    currentUser && Array.isArray(opportunity.skills)
      ? calculateMatchScore(currentUser.skills || [], opportunity.skills)
      : 0
  ...
  const handleApply = async () => {
    ...
    try {
      const response = await fetch(
        `${backendBase}/api/internships/${opportunity.id}/apply`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId: user.id }),
  ...
  const handleSelect = async (applicationId: string) => {
    try {
      await fetch(
        `${backendBase}/api/internships/applications/${applicationId}/select`,
        { method: "PUT" }
      )
      setOpportunity((prev: any) => ({
        ...prev,
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="glass rounded-2xl p-6">
            ...
            <div className="space-y-4">
              ...
              {/* Eligibility — from File 1 */}
              {opportunity.eligibility &&
                (opportunity.eligibility.minCGPA ||
                  opportunity.eligibility.departments?.length > 0) && (
                  <div>
                    <h2 className="mb-2 font-semibold text-foreground">
                      Eligibility
                    </h2>
          ...
          {/* ── Staff: Internship Applicants Table — from File 1 ── */}
          {isStaff && opportunity.type === "internship" && (
            <Card className="glass rounded-2xl p-6 border-primary/20 bg-primary/5">
              <div className="flex items-center gap-2 mb-4">
                <UsersIcon className="h-6 w-6 text-primary" />
                <h2 className="text-xl font-bold text-primary">
                        ...
                        >
                          ...
                          <td className="px-4 py-4 text-muted-foreground whitespace-nowrap">
                            <div className="flex items-center gap-1.5">
                              ...
                              <a
                                href={`mailto:${app.student?.primary_email}`}
          ...
          {/* ── Staff: Project Groups Table — from File 2 ── */}
          {isStaff && opportunity.type === "project" && (
            <Card className="glass rounded-2xl p-6 border-primary/20 bg-primary/5">
              <div className="flex items-center gap-2 mb-4">
                <UsersIcon className="h-6 w-6 text-primary" />
                          ...
                          {group.status === "PENDING" ? (
                            <>
                              <Button
                                size="sm"
                                onClick={() =>
                                  handleGroupStatus(group.group_id, "ACCEPTED")
                                }
                                className="bg-green-600 hover:bg-green-700 text-white"
                              >
                                Accept
                              ...
                              <Button
                                ...
                                variant="destructive"
                                onClick={() =>
                                  handleGroupStatus(group.group_id, "REJECTED")
                                }
                              >
                                Reject
                              </Button>
                        ...
                        <tbody className="divide-y divide-border">
                          {group.members.map((member: any) => (
                            ...
                            >
                              ...
                              <td className="px-3 py-3 text-muted-foreground whitespace-nowrap">
                                <a
                                  href={`mailto:${member.primary_email}`}
                                  className="hover:text-primary transition-colors"
                                >
                                  {member.primary_email}
                                </a>
                              ...
                              <td className="px-3 py-3">
                                {group.leader?.registration_number ===
            ...
            {isStaff ? (
              <div className="text-center py-4 bg-primary/10 rounded-xl border border-primary/20">
                ...
                <p className="text-xs text-muted-foreground">
                  You are viewing administrative details.
                </p>
              </div>
            ) : isPlaced ? (
              <div className="text-center py-4">
